...
- **Próximo passo:** comprar dongle BT USB com SCO MTU RX > 0 (ex: Realtek RTL8761B, lsusb 0bda:8771 — suportado kernel 5.8+ via btrtl; nosso host kernel 7.0.x OK). EVITAR CSR 0a12:0001 (CSR8510 antigo, SCO MTU RX=0) e BCM20702 (firmware chato no Linux). Ao chegar: `hciconfig -a hciX` → checar `SCO MTU: X:Y` onde Y > 0. Se Y=0, retornar.
  - **AliExpress:** buscar "Realtek RTL8761B USB Bluetooth 5.0 adapter" ou "0bda:8771". Modelos conhecidos com RTL8761B: TP-Link UB500, UGREEN BT5.0 (alguns), C-TECH BTD-01, Orico. **NÃO comprar** "Bluetooth audio receiver/transmitter" com jack 3.5mm — NÃO é HCI dongle (não expõe hciX pro host, inútil pra HFP).
